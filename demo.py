...
model = modellib.AttResnet101FCN(mode="inference", model_dir=MODEL_DIR, config=config)
...
att_prob = model.run_graph([image],model.get_layers_by_name(['mask_att_prob']))
att_prob=scipy.ndimage.zoom(att_prob['mask_att_prob'][0], zoom=[8,8,1])
molded_images, image_metas, windows =model.mold_inputs([image])
a_prob,_ = model.unmold_detections(att_prob,att_prob[...,0],image.shape,windows[0])
print(a_prob.sum(2))
for v in range(3):
    plt.subplot(1,3,v+1)
